---
title: Style - Concepts - Handbook - Apache ECharts
source_url: https://echarts.apache.org/handbook/en/concepts/style
target_id: apache-echarts
dependency: Apache ECharts
collected_at: 2026-04-16T03:22:47.123781+00:00
kind: extracted-doc
---
...
- 2026-04-16T03:22:47.123781+00:00
...
- These approaches below will be introduced.
...
This article provides an overview of the different approaches about Apache ECharts

TM

style customization. For example, how to config the color, size, shadow of the graphic elements and labels.
...
, we can switch to dark mode and see the result of a different theme.
...
Other themes are not included by default, and need to load them ourselves if we want to use them. Themes can be visited and downloaded in the

theme builder

. Theme can also be created or edited in it. The downloaded theme can be used as follows:
...
It is a common way to set style explicitly. Throughout ECharts

option

, style related options can be set in various place, including
...
Generally speaking, all of the built-in components and series follow the naming convention like
...
etc, although they may occur in different place according to different series or components.
...
When mouse hovering a graphic elements, usually the emphasis style will be displayed. By default, the emphasis style is auto generated by the normal style. However they can be specified by
...
is the same as the ones for normal state, for example:
...
The option format is still

compatible

, but not recommended. In fact, in most cases, users only set normal style, and use the default emphasis style. So since ECharts4, we support to write style without the "normal" term, which makes the option more simple and neat.

Visual Encoding by visualMap Component

visualMap component

supports config the rule that mapping value to visual channel (color, size, ...). More details can be check in
