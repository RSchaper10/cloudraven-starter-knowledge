...
source_url: https://threejs.org/docs/pages/TSL.html
target_id: three-js
dependency: Three.js
collected_at: 2026-04-16T03:23:36.161133+00:00
kind: extracted-doc
---
...
- 2026-04-16T03:23:36.161133+00:00

Direct links in scope:

- https://threejs.org/docs/pages/TSL.html
- https://threejs.org/docs/pages/Node.html
- https://threejs.org/docs/pages/PropertyNode.html
- https://threejs.org/docs/pages/UniformNode.html
- https://threejs.org/docs/pages/VaryingNode.html
- https://threejs.org/docs/pages/ViewportDepthNode.html
- https://threejs.org/docs/pages/IndexNode.html
- https://threejs.org/docs/pages/UniformGroupNode.html
- https://threejs.org/docs/pages/FrontFacingNode.html
- https://threejs.org/docs/pages/ComputeBuiltinNode.html
- https://threejs.org/docs/pages/ModelNode.html
- https://threejs.org/docs/pages/PointUVNode.html
- https://threejs.org/docs/pages/AttributeNode.html
- https://threejs.org/docs/pages/VelocityNode.html
- https://threejs.org/docs/pages/ScreenNode.html
- https://threejs.org/docs/pages/RendererReferenceNode.html
- https://threejs.org/docs/pages/ExpressionNode.html
- https://threejs.org/docs/pages/VarNode.html
- https://threejs.org/docs/pages/ConditionalNode.html
- https://threejs.org/docs/pages/global.html

Captured summary:

- EPSILON : Node .<float> (constant) A small value used to handle floating-point precision errors.
- HALF_PI : Node .<float> (constant) Represents PI / 2.
- INFINITY : Node .<float> (constant) Represents infinity.
- PI : Node .<float> (constant) Represents PI.
- PI2 : Node .<float> (constant) Represents PI * 2.
...
Represents PI * 2. Please use the non-deprecated version
...
TSL object that represents the shader variable
...
value of the camera used for the current render.
...
TSL object that represents the current

index

value of the camera if used ArrayCamera.
...
If the material is set to

BackSide

, the direction is inverted. If the material is set to

DoubleSide

, the direction is multiplied by
...
TSL object that represents the object's model normal view in

highp

precision which is achieved by computing the matrix in JS and not in the shader.
...
TSL object that represents the object's model view in
...
TSL object that represents the ambient occlusion map of the current material. The value is composed via
...
TSL object that represents the clearcoat of the current material. The value is composed via
...
TSL object that represents the clearcoat normal of the current material. The value will be either
...
TSL object that represents the clearcoat roughness of the current material. The value is composed via
...
TSL object that represents the diffuse color of the current material. The value is composed via
...
TSL object that represents the emissive color of the current material. The value is composed via
...
TSL object that represents the intensity of environment maps of PBR materials. When
...
TSL object that represents the rotation of environment maps. When
...
TSL object that represents the light map of the current material. The value is composed via
...
TSL object that represents the metalness of the current material. The value is composed via
...
TSL object that represents the normal of the current material. The value will be either
...
TSL object that represents the opacity of the current material. The value is composed via
...
TSL object that represents the roughness of the current material. The value is composed via
...
TSL object that represents the sheen color of the current material. The value is composed via
...
TSL object that represents the sheen roughness of the current material. The value is composed via
...
TSL object that represents the specular color of the current material. The value is composed via
...
TSL object that represents the specular intensity of the current material. The value is composed via
...
TSL object that represents the specular strength of the current material. The value is composed via
...
TSL object that represents the thickness of the current material. The value is composed via
...
TSL object that represents the transmission of the current material. The value is composed via
...
TSL object that represents the previous vertex position in local space of the current rendered object. Used in context of
...
pixel position on the screen in physical pixel units.
...
TSL object that represents normalized screen coordinates, unitless in
...
since r178. Use `clearcoatNormalView` instead.
...
since r178. Use `normalView` instead.
...
since r178. Use `normalWorld` instead.
...
TSL object that represents the viewport rectangle as
...
pixel position on the viewport in physical pixel units.
...
TSL object that represents normalized viewport coordinates, unitless in
...
// Execute 12 compute threads with a workgroup size of 3. const computeFn = Fn( () => { If( workgroupId.x.mod( 2 ).equal( 0 ), () => { storageBuffer.element( instanceIndex ).assign( instanceIndex ); } ).Else( () => { storageBuffer.element( instanceIndex ).assign( 0 ); } ); } )().compute( 12, [ 3 ] ); // workgroupId.x = [0, 0, 0, 1, 1, 1, 2, 2, 2, 3, 3, 3]; // Buffer Output = [0, 1, 2, 0, 0, 0, 6, 7, 8, 0, 0, 0];
...
If( condition, function ) .ElseIf( condition, function ) .Else( function )
...
Switch( value ) .Case( 1, function ) .Case( 2, 3, 4, function ) .Default( function )
...
Returns the arc-tangent of the parameter. If two parameters are provided, the result is
...
TSL function for creating a storage buffer node with a configured
...
1.0 if inside bounds, 0.0 if outside.
...
This function blends two color based on their alpha values by replicating the behavior of
...
, this mode also lightens the base layer's colors based on the color of the blend layer. The "Screen" blend mode is better for general brightening whereas the "Dodge" results in more subtle and nuanced effects.
...
parameter should be small (1, 2 or 3) since it determines the number of samples based on (size * 2 + 1)^2. This implementation uses a single pass approach so the kernel is not applied as a separable filter. That means larger kernels won't perform well. Use Gaussian instead if you need a more high-quality blur.

To produce wider blurs, increase the

separation

parameter instead which has no influence on the performance.
...
True if the circle intersects the AABB.
...
The color with bleeding effect applied.
...
A direction vector for sampling which is by default
...
This class can be used to configure a fog for the scene. Nodes of this type are assigned to
...
interval. Expands the sides and compresses the center, and keeps
...
const uvNode = getParallaxCorrectNormal( reflectVector, vec3( 200, 100, 100 ), vec3( 0, - 50, 0 ) ); material.envNode = pmremTexture( renderTarget.texture, uvNode );
...
This function checks if a shadow material already exists for the provided light. If not, it creates a new

NodeMaterial

configured for shadow rendering and stores it in the
...
The shadow material associated with the given light.
...
A function that renders shadow objects.
...
Generates a hash value in the range
...
The approach of this blur is different compared to Gaussian and box blur since it does not rely on a kernel to apply a convolution. Instead, it reads the base texture multiple times in a random pattern and then averages the samples. A typical artifact of this technique is a slightly noisy appearance of the blur which can be mitigated by increasing the number of iterations (see

repeats

parameter). Compared to Gaussian blur, hash blur requires just a single pass.
...
TSL function for creating an instance of

LightsNode

and configuring it with the given array of lights.
...
The perspective depth. If

null

is provided, the current fragment's depth is used.
...
Can be used to compute texture coordinates for projecting a matcap onto a mesh. Used by
...
The input node with the motion blur effect applied.
...
interval. The corners are mapped to
...
Allows to control the remapping functions shape by rising the parabola to a power
...
interval. A generalization of the

parabola()

. Keeps the corners mapped to 0 but allows the control of the shape one either side of the curve. Reference:
...
since r180. Use `gaussianBlur()` with `premultipliedAlpha: true` option instead.
...
The value received from the swap operation.
...
Base weight factor for each sample in the range
...
Decreases the weight factor so each iteration adds less to the sum. Must be in the range
...
The number if iterations. Should be in the range
...
material.contextNode = replaceDefaultUV( ( textureNode ) => { // ... return customUVCoordinates; } );
...
A context node that replaces the default UV coordinates.
...
// Combined with other effects let pipeline = retroPass( scene, camera ); pipeline = bayerDither( pipeline, 32 ); pipeline = posterize( pipeline, 32 ); renderPipeline.outputNode = pipeline;
...
Specifies the amount of the conversion. A value under

1

desaturates the color, a value over
...
The color with scanlines applied.
...
A phase shifted sinus curve that starts at zero and ends at zero, with bouncing behavior. Reference:
...
const uvNode = spritesheetUV( vec2( 6, 6 ), uv(), time.mul( animationSpeed ) ); material.colorNode = texture( spriteSheet, uvNode );
...
A node object wrapping the SubBuildNode instance.
...
The accumulated result of the reduction operation.
...
The accumulated result of the exclusive scan operation.
...
The accumulated result of the inclusive scan operation.
...
should influence the transition or not.
...
Second texture node. When not set, the shader will sample from
...
Third texture node. When not set, the shader will sample from
...
The color with non-premultiplied alpha.
...
Optional uv coordinates. By default
...
The color with vignette applied.
...
that represents the current loading status.
...
Only relevant for objects using multiple materials. This represents a group entry from the respective
