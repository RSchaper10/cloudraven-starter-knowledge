...
- Creating a scene Creating a scene The goal of this section is to give a brief introduction to three.js.
...
- Before we start If you haven't yet, go through the `Installation` guide.
- We'll assume you've already set up the same project structure (including index.html and main.js ), have installed three.js, and are either running a build tool, or using a local server with a CDN and import maps.
...
function animate( time ) { renderer.render( scene, camera ); } renderer.setAnimationLoop( animate );
...
The thing is - we could, but `requestAnimationFrame` which is internally used in `WebGLRenderer` has a number of advantages. Perhaps the most important one is that it pauses when the user navigates to another browser tab, hence not wasting their precious processing power and battery life.
