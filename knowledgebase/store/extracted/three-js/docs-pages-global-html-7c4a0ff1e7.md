---
title: Global - Three.js Docs
source_url: https://threejs.org/docs/pages/global.html
target_id: three-js
dependency: Three.js
collected_at: 2026-04-16T03:23:22.023978+00:00
kind: extracted-doc
---

# Global - Three.js Docs
...
- 2026-04-16T03:23:22.023978+00:00
...
- https://threejs.org/docs/pages/EventNode.html
- https://threejs.org/docs/pages/DataTexture.html
- https://threejs.org/docs/pages/RenderContext.html
...
- Global - Three.js Docs Global Properties .
- ACESFilmicToneMapping : number (constant) ACES Filmic tone mapping.
- AddEquation : number (constant) A source + destination blending equation.
- AddOperation : number (constant) Adds the two colors.
- AdditiveAnimationBlendMode : number (constant) Additive animation blend mode.
...
Decrements the current stencil buffer value. Wraps stencil buffer value to the maximum representable unsigned value when decrementing a stencil buffer value of
...
Reads each element as a single depth value, converts it to floating point, and clamps to the range
...
Reads each element is a pair of depth and stencil values. The depth component of the pair is interpreted as in
...
Returns the weighted average of the four texture elements that are closest to the specified texture coordinates, and can include items wrapped or repeated from other parts of a texture, depending on the values of
...
criterion to produce a texture value from each mipmap. The final texture value is a weighted average of those two values.
...
criterion (a weighted average of the four texels that are closest to the center of the pixel) to produce a texture value.
...
The update method is executed per

Object3D

that uses the node for rendering.
...
The event will be bound to the declared TSL function
...
or the JS function call must be inherited from one.
...
minus the destination alpha value. The alpha value is multiplied by
...
A shadow filtering function performing basic filtering. This is in fact an unfiltered version of the shadow map with a binary
...
The node that was added to the stack.
...
The value to find a POT for. Must be greater than
...
Scales the texture as large as possible within its surface without cropping or stretching the texture. The method preserves the original aspect ratio of the texture. Akin to CSS
...
Scales the texture to the smallest possible size to fill the surface, leaving no empty space. The method preserves the original aspect ratio of the texture. Akin to CSS
...
A canvas element with display set to 'block'.
...
Smoothly interpolate a number from
...
in a spring-like manner using a delta time to maintain frame rate independent movement. For details, see
...
The denormalize (float) value in the range
...
The filtered and enhanced message parameters.
...
Computes the Euclidean modulo of the given parameters that is
...
Configures the texture to the default transformation. Akin to CSS
...
The custom console function, or null if not set.
...
: Array.<{name: string, type: string, atomic: boolean}>
...
This function is called with a vector length and ensure the computed length matches a predefined stride (in this case
...
The uniforms object to be used in the shader.
...
Returns a node that represents the

z

coordinate in view space for the current fragment. It's a different representation of the default depth value.
...
Returns the percentage in the closed interval
...
if the given object is a typed array.
...
Returns a value that alternates between
...
The created SampleNode instance wrapped as a node object.
...
Returns a deterministic pseudo-random float in the interval
...
Sets the given quaternion from the

Intrinsic Proper Euler Angles

defined by the given angles and order.

Rotations are applied to the axes in the order specified by order: rotation by angle

a

is applied first, then by angle
...
variation on smoothstep

that has zero 1st and 2nd order derivatives at
...
The value to evaluate based on its position between
...
that represents the percentage that
...
, but smoothed or slowed down the closer
...
Sorts the given array by the previously computed order via
