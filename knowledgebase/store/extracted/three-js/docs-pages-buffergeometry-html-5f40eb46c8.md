---
title: BufferGeometry - Three.js Docs
source_url: https://threejs.org/docs/pages/BufferGeometry.html
target_id: three-js
dependency: Three.js
collected_at: 2026-04-16T03:23:24.820073+00:00
kind: extracted-doc
---
...
- 2026-04-16T03:23:24.820073+00:00
...
- https://threejs.org/docs/pages/InterleavedBufferAttribute.html
- https://threejs.org/docs/pages/Box3.html
- https://threejs.org/docs/pages/Sphere.html
...
- BufferGeometry - Three.js Docs EventDispatcher → BufferGeometry Constructor new BufferGeometry () Constructs a new geometry.
- attributes : Object.<string, ( BufferAttribute | InterleavedBufferAttribute )> This dictionary has as id the name of the attribute to be set and as value the buffer attribute to set it to.
...
This dictionary has as id the name of the attribute to be set and as value the buffer attribute to set it to. Rather than accessing this property directly, use
...
to access attributes of this geometry.
...
to edit groups, rather than modifying this array directly.
...
Note: Once the geometry has been rendered, the morph attribute data cannot be changed. You will have to call

dispose()

, and create a new geometry instance.
...
Used to control the morph target behavior; when set to

true

, the morph target data is treated as relative offsets, rather than as absolute positions/normals.
...
member. The bounding box is not computed by the engine; it must be computed by your app. You may need to recompute the bounding box if the geometry vertices are modified.
...
member. The engine automatically computes the bounding sphere when it is needed, e.g., for ray casting or view frustum culling. You may need to recompute the bounding sphere if the geometry vertices are modified.
...
The buffer attribute. Returns

undefined

if not attribute has been found.
...
The indirect attribute. Returns

null

if no indirect attribute is defined.
...
if this geometry has an attribute for the given name.
...
Rotates the geometry to face a point in 3D space. This is typically done as a one time operation, and not during a loop. Use
...
Ensures every normal vector in a geometry will have a magnitude of
...
Rotates the geometry about the X axis. This is typically done as a one time operation, and not during a loop. Use
...
Rotates the geometry about the Y axis. This is typically done as a one time operation, and not during a loop. Use
...
Rotates the geometry about the Z axis. This is typically done as a one time operation, and not during a loop. Use
...
Scales the geometry. This is typically done as a one time operation, and not during a loop. Use
...
is the number of vertices to render. For indexed BufferGeometry,

count

is the number of indices to render.
...
attribute based on the given array of points. The array can hold 2D or 3D vectors. When using two-dimensional data, the

z

coordinate for all vertices is set to
...
Translates the geometry. This is typically done as a one time operation, and not during a loop. Use
