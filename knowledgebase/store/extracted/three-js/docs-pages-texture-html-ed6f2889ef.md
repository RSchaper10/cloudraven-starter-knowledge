---
title: Texture - Three.js Docs
source_url: https://threejs.org/docs/pages/Texture.html
target_id: three-js
dependency: Three.js
collected_at: 2026-04-16T03:23:37.840854+00:00
kind: extracted-doc
---
...
- 2026-04-16T03:23:37.840854+00:00
...
- https://threejs.org/docs/pages/Source.html
- https://threejs.org/docs/pages/ObjectLoader.html

Captured summary:

- Texture - Three.js Docs EventDispatcher → Texture Constructor new Texture ( image : Object , mapping : number , wrapS : number , wrapT : number , magFilter : number , minFilter : number , format : number , type : number , anisotropy : number , colorSpace : string ) Constructs a new texture.
- image The image holding the texture data.
- anisotropy : number The number of samples taken along the axis through the pixel that has the highest density of texels.
- A higher value gives a less blurry result than a basic mipmap, at the cost of more texture samples being used.
- center : Vector2 The point around which rotation occurs.
...
The number of samples taken along the axis through the pixel that has the highest density of texels. By default, this value is
...
The point around which rotation occurs. A value of

(0.5, 0.5)

corresponds to the center of the texture. Default is
...
Note that this property has no effect when using
...
if you are creating mipmaps manually.
...
The default internal format is derived from
...
and defines how the texture data is going to be stored on the GPU.
...
How the texture is applied to the object. The value
...
Whether to update the texture's uv-transformation
...
indicates the engine the PMREM must be regenerated.
...
indicates the engine the texture must be updated in the next render. This triggers a texture upload to the GPU and ensures correct texture parameter configuration.
...
How much a single repetition of the texture is offset from the beginning, in each direction U and V. Typical range is
...
or not (only relevant for render target textures).
...
in either direction, the corresponding wrap parameter should also be set to
...
This can be used to only update a subregion or specific rows of the texture (for example, just the first 3 rows). Use the
...
Sets this texture's properties based on
...
Updates the texture transformation matrix from the from the properties
