---
title: Renderer - Three.js Docs
source_url: https://threejs.org/docs/pages/Renderer.html
target_id: three-js
dependency: Three.js
collected_at: 2026-04-16T03:23:33.574249+00:00
kind: extracted-doc
---

# Renderer - Three.js Docs
...
- 2026-04-16T03:23:33.574249+00:00

Direct links in scope:

- https://threejs.org/docs/pages/Renderer.html
- https://threejs.org/docs/pages/Backend.html
- https://threejs.org/docs/pages/ContextNode.html
- https://threejs.org/docs/pages/global.html
- https://threejs.org/docs/pages/Info.html
- https://threejs.org/docs/pages/InspectorBase.html
- https://threejs.org/docs/pages/NodeLibrary.html
- https://threejs.org/docs/pages/Lighting.html
- https://threejs.org/docs/pages/XRManager.html
...
- https://threejs.org/docs/pages/Vector4.html
- https://threejs.org/docs/pages/Texture.html
- https://threejs.org/docs/pages/Box2.html
- https://threejs.org/docs/pages/Box3.html

Captured summary:

- Renderer - Three.js Docs Renderer Constructor new Renderer ( backend : Backend , parameters : Renderer~Options ) Constructs a new renderer.
- backend The backend the renderer is targeting (e.g.
- alpha : boolean Whether the default framebuffer should be transparent or opaque.
- autoClear : boolean Whether the renderer should automatically clear the current rendering target before execute a render() call.
- The target can be the canvas (default framebuffer) or the current bound render target (custom framebuffer).
...
call. The target can be the canvas (default framebuffer) or the current bound render target (custom framebuffer).
...
, this property defines whether the renderer should clear the color buffer.
...
, this property defines whether the renderer should clear the depth buffer.
...
, this property defines whether the renderer should clear the stencil buffer.
...
The coordinate system of the renderer. The value of this property depends on the selected backend. Either
...
The current tone mapping of the renderer. When not producing screen output, the tone mapping is always
...
NOTE: 64-bit precision is not compatible with
...
The inspector instance. The inspector can be any class that extends from
...
if the rendering settings are set to screen output.
...
The node library defines how certain library objects like materials, lights or tone mapping functions are mapped to node types. This is required since although instances of classes like
...
can be part of the scene graph, they are internally represented as nodes for further processing.
...
if a framebuffer target is needed to perform tone mapping or color space conversion. If this is the case, the renderer allocates an internal render target for that purpose.
...
Performs a manual clear operation. This method ignores
...
Performs a manual clear operation of the color buffer. This method ignores
...
Performs a manual clear operation of the depth buffer. This method ignores
...
Performs a manual clear operation of the stencil buffer. This method ignores
...
The current animation loop callback.
...
since r182. Use `.getOutputBufferType()` instead.
...
: GPUCanvasContext | WebGL2RenderingContext
...
The current output render target. Returns

null

if no output target is set.
...
The current render object function. Returns
...
Checks if the given compatibility is supported by the selected backend. If the renderer has not been initialized, this method always returns
...
Whether the compatibility is supported or not.
...
Checks if the given feature is supported by the selected backend. If the renderer has not been initialized, this method always returns
...
This method performs an occlusion query for the given 3D object. It returns

true

if the given 3D object is fully occluded by other 3D objects in the scene.
...
coordinate of the copy region's origin.
...
Renders the scene or 3D object with the given camera. This method can only be called if the renderer has been initialized. When using

render()

inside an animation loop, it's guaranteed the renderer will be initialized. The animation loop must be defined with
...
The target of the method is the default framebuffer (meaning the canvas) or alternatively a render target when specified via
...
Applications are advised to always define the animation loop with this method and not manually with
...
size.x = width * pixelRatio; size.y = height * pixelRatio;
...
Defines a manual sort function for the opaque render list. Pass

null

to use the default sort.
...
Sets the given render object function. Calling this method overwrites the default implementation which is

Renderer#renderObject

. Defining a custom function can be useful if you want to modify the way objects are rendered. For example you can define things like "every object that has material of a certain type should perform a pre-pass with a special overwrite material". The custom function must always call
...
as the first argument to reset the state.
...
Sets the given render target. Calling this method means the renderer does not target the default framebuffer (meaning the canvas) anymore but a custom framebuffer. Use
...
attribute of the canvas or not.
...
Defines a manual sort function for the transparent render list. Pass
...
samples are used by default. This parameter can set to any other integer value than 0 to overwrite the default.
