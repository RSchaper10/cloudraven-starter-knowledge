---
title: Object3D - Three.js Docs
source_url: https://threejs.org/docs/pages/Object3D.html
target_id: three-js
dependency: Three.js
collected_at: 2026-04-16T03:23:21.479113+00:00
kind: extracted-doc
---
...
- 2026-04-16T03:23:21.479113+00:00
...
- https://threejs.org/docs/pages/Group.html
...
- Object3D - Three.js Docs EventDispatcher → Object3D Constructor new Object3D () Constructs a new 3D object.
...
- customDepthMaterial : Material | undefined Custom depth material to be used when rendering to the depth map.
...
, the 3D object gets rendered into shadow maps.
...
Custom depth material to be used when rendering to the depth map. Can only be used in context of meshes. When shadow-casting with a
...
, if you are modifying vertex positions in the vertex shader you must specify a custom depth material for proper shadows.

Only relevant in context of
...
, the 3D object is honored by view frustum culling.
...
The layer membership of the 3D object. The 3D object is only visible if it has at least one layer in common with the camera in use. This property can also be used to filter out unwanted objects in ray-intersection tests when using
...
, the engine automatically computes the local matrix from position, rotation and scale every frame. If set to

false

, the app is responsible for recomputing the local matrix by calling

updateMatrix()

.

The default values for all 3D objects is defined by
...
, the engine automatically computes the world matrix from the current local matrix and the object's transformation hierarchy. If set to

false

, the app is responsible for recomputing the world matrix by directly updating the
...
, it calculates the world matrix in that frame and resets this property to
...
, the 3D object is affected by shadows in the scene.
...
This value allows the default rendering order of scene graph objects to be overridden although opaque and transparent objects remain sorted independently. When this property is set for an instance of

Group

,all descendants objects will be sorted and rendered together. Sorting is from lowest to highest render order.
...
Whether the 3D object is supposed to be static or not. If set to

true

, it means the 3D object is not going to be changed after the initial renderer. This includes geometry and material settings. A static 3D object can be processed by the renderer slightly faster since certain state checks can be bypassed.
...
direction of the 3D object which influences the orientation via methods like
...
Object3D#event:added

Object3D#event:childadded
...
Object3D#event:removed

Object3D#event:childremoved
...
, descendants of the 3D object are also cloned.
...
, descendants of the 3D object are cloned.
...
The found 3D object. Returns

undefined

if no 3D object has been found.
...
Abstract method to get intersections between a casted ray and this 3D object. Renderable 3D objects such as
...
, but the callback will only be executed for all ancestors.
...
, but the callback will only be executed for visible 3D objects. Descendants of invisible 3D objects are not traversed.
...
To ensure correct results, this method also recomputes the 3D object's transformation matrix in local space. The computation of the local and world matrix can be controlled with the
...
flags which are both

true

by default. Set these flags to

false

if you need more control over the update matrix process.
...
, a recomputation of world matrices is forced even when
...
with more control over the update of ancestor and descendant nodes.
