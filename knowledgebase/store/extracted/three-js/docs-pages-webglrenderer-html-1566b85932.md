---
title: WebGLRenderer - Three.js Docs
source_url: https://threejs.org/docs/pages/WebGLRenderer.html
target_id: three-js
dependency: Three.js
collected_at: 2026-04-16T03:23:24.064435+00:00
kind: extracted-doc
---
...
- 2026-04-16T03:23:24.064435+00:00
...
- WebGLRenderer - Three.js Docs WebGLRenderer Constructor new WebGLRenderer ( parameters : WebGLRenderer~Options ) Constructs a new WebGL renderer.
...
, whether the renderer should clear the color buffer or not.
...
, whether the renderer should clear the depth buffer or not.
...
, whether the renderer should clear the stencil buffer or not.
...
: Used to check whether a WebGL extension is supported and return the extension object if available.
...
By default these data are reset at each render call but when having multiple render passes per frame (e.g. when using post processing) it can be preferred to reset with a custom pattern. First, set
...
The normalized resolution scale for the transmission render target, measured in percentage of viewport dimensions. Lowering this value can result in significant performance improvements when using
...
Clears the color buffer. Equivalent to calling
...
Clears the depth buffer. Equivalent to calling
...
Clears the stencil buffer. Equivalent to calling
...
WebGL extension. Hence, it is recommended to use this version of
...
When using render target textures as
...
, you must make sure both render targets are initialized
...
Simulates a loss of the WebGL context. This requires support for the
...
Simulates a restore of the WebGL context. This requires support for the
...
Returns the clear alpha. Ranges within
...
The active render target. Returns

null

if no render target is currently set.
...
Initializes the given WebGLRenderTarget memory. Useful for initializing a render target so data can be copied into it using
...
Asynchronous, non-blocking version of

WebGLRenderer#readRenderTargetPixels

.

It is recommended to use this version of
...
The render is done to a previously specified render target set by calling
...
. If you want to prevent only certain buffers being cleared you can
...
Sets a custom opaque sort function for the render lists. Pass
...
Sets the scissor region to render from
...
Resizes the output canvas to (width, height) with device pixel ratio taken into account, and also sets the viewport to fit that size, starting in (0, 0). Setting
...
Sets a custom transparent sort function for the render lists. Pass
...
Sets the viewport to render from
...
in the constructor and the rendering context supports
...
The default shader precision. Uses
...
Controls the default clear alpha value. When set to
...
Whether to use a logarithmic depth buffer. It may be necessary to use this if dealing with huge differences in scale in a single scene. Note that this setting uses

gl_FragDepth

if available which disables the Early Fragment Test optimization and can cause a decrease in performance.
...
Whether to use a reverse depth buffer. Requires the

EXT_clip_control

extension. This is a more faster and accurate version than logarithmic depth buffer.
...
Defines the type of the output buffer. Use

HalfFloatType

for HDR rendering with tone mapping and post-processing support.
...
Enables automatic updates to the shadows in the scene. If you do not require dynamic lighting / shadows, you may set this to
...
, shadow maps in the scene will be updated in the next
