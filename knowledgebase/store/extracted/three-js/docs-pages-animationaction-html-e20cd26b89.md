---
title: AnimationAction - Three.js Docs
source_url: https://threejs.org/docs/pages/AnimationAction.html
target_id: three-js
dependency: Three.js
collected_at: 2026-04-16T03:23:19.183545+00:00
kind: extracted-doc
---

# AnimationAction - Three.js Docs
...
- 2026-04-16T03:23:19.183545+00:00
...
- AnimationAction - Three.js Docs AnimationAction Constructor new AnimationAction ( mixer : AnimationMixer , clip : AnimationClip , localRoot : Object3D , blendMode : NormalAnimationBlendMode | AdditiveAnimationBlendMode ) Constructs a new animation action.
...
will automatically be switched to

false

when the last loop of the action has finished, so that this action has no further impact.
...
, the action is disabled so it has no impact.

When the action is re-enabled, the animation continues from its current time (setting
...
, the playback of the action is paused.
...
The local time of this action (in seconds, starting with

0

).

The value gets clamped or wrapped to

[0,clip.duration]

(according to the loop state).
...
causes the animation to pause. Negative values cause the animation to play backwards.
...
The degree of influence of this action (in the interval
...
(full impact) can be used to blend between several actions.
...
Fades the animation in by increasing its weight gradually from
...
Fades the animation out by decreasing its weight gradually from
