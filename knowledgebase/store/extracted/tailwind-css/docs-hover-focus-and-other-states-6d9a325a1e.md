...
- 2026-04-16T03:22:03.300934+00:00
...
- It does nothing by default, but as soon as you hover over an element with that class, the background color will change to sky-700 .
...
by adding a variant to the beginning of the class name that describes the condition you want to target.

For example, to apply the

bg-sky-700

class on hover, use the
...
defines styles for the

:hover

state? It does nothing by default, but as soon as you hover over an element with that class, the background color will change to

sky-700

.

This is what we mean when we say a utility class can be applied
...
, like responsive breakpoints, dark mode, and
...
Style elements on hover, focus, and active using the
...
Style an element when it is the first-child or last-child using the
...
You can also style an element when it's an odd or even child using the
...
variants to style children based on their position in the list:
...
You can pass any number you want to these by default, and use arbitrary values for more complex expressions like

nth-[2n+1_of_li]

.

Tailwind also includes variants for other structural pseudo-classes like
...
Style form elements in different states using variants like
...
Tailwind also includes variants for other form states like
...
variant to style an element based on the state or content of its descendants:
...
with a pseudo-class, like

has-[:focus]

, to style an element based on the state of its descendants. You can also use element selectors, like
...
If you need to style an element based on the descendants of a parent element, you can mark the parent with the
...
variant to style the target element:
...
If you need to style an element based on the descendants of a sibling element, you can mark the sibling with the
...
variant to style an element when a condition is not true.

It's particularly powerful when combined with other pseudo-class variants, for example combining
...
to only apply hover styles when an element is not focused:
...
variant with media query variants like
...
to only style an element when something about the user's environment is not true:
...
When you need to style an element based on the state of some

parent

element, mark the parent with the
...
This pattern works with every pseudo-class variant, for example
...
When nesting groups, you can style something based on the state of a

specific

parent group by giving that parent a unique group name using a

group/{name}

class, and including that name in variants using classes like
...
You can create one-off

group-*

variants on the fly by providing your own selector as an
...
variant works similarly to

group

except you don't need to add
...
"opacity-50 group-focus:opacity-100"
...
variant responds to state changes in any parent, so if you want more fine-grained control you'll need to use
...
When you need to style an element based on the state of a

sibling

element, mark the sibling with the
...
This makes it possible to do all sorts of neat tricks, like
...
marker can only be used on

previous

siblings because of how the
...
When using multiple peers, you can style something on the state of a

specific

peer by giving that peer a unique name using a
...
When using these variants, Tailwind will automatically add
...
"text-center text-2xl font-semibold text-gray-900 italic dark:text-white"
...
"relative text-white dark:text-gray-950"
...
For example, here's the same design from above but using a
...
"text-center text-2xl font-semibold text-gray-900 italic"
...
"absolute -inset-1 block -skew-y-3 bg-pink-500"
...
Style the placeholder text of any input or textarea using the
...
Style the button in file inputs using the
...
variant to be inheritable, so although you can use it directly on an
...
Style the active text selection using the
...
living things but I tell you Jerry at that moment, I
...
Style the first line in a block of content using the

first-line

variant, and the first letter using the
...
If you're using native

<dialog>

elements in your project, you may also want to read about
...
To style an element at a specific breakpoint, use responsive variants like
...
To style an element based on the width of a parent element instead of the viewport, use variants like
...
Use utilities with no variant to target light mode, and use the

dark

variant to provide overrides for dark mode:
...
variant to conditionally add styles when the user has requested reduced motion:
...
Tailwind also includes a

motion-safe

variant that only adds styles when the user has

not

requested reduced motion. This can be useful when using the
...
variant to conditionally add styles when the user has requested more contrast:
...
contrast-more:placeholder-gray-500
...
variant you can use to conditionally add styles when the user has requested less contrast.
...
variant to conditionally add styles when the user has enabled a forced color mode:
...
variant to apply styles based when the user is

not

using a forced colors mode:
...
variant to conditionally add styles when the user has enabled an inverted color scheme:
...
media query tells you whether the user has a primary pointing device, like a mouse, and the accuracy of that pointing device.

Use the

pointer-fine

variant to target an accurate pointing device, like a mouse or trackpad, or the

pointer-coarse

variant to target a less accurate pointing device, like a touchscreen, which can be useful for providing larger click targets on touch devices:
...
only targets the primary pointing device,

any-pointer

is used to target any of the pointing devices that might be available. Use the
...
variants to provide different styles if at least one connected pointing device meets the criteria.
...
variants to conditionally add styles when the viewport is in a specific orientation:
...
variant to conditionally add styles based on whether the user has scripting, such as JavaScript, enabled:
...
This experience requires JavaScript to function. Please enable JavaScript in your browser settings.
...
variant to conditionally add styles that only apply when the document is being printed:
...
variant to style things based on whether a certain feature is supported in the user's browser:
...
and takes anything you’d use with

@supports (...)

between the square brackets, like a property/value pair, and even expressions using
...
variant to style things based on whether a certain feature is not supported in the user's browser:
...
You can configure shortcuts for common

@supports

rules you're using in your project by creating a new variant in the
...
variant to set the appearance of an element when it is first rendered in the DOM, or transitions from
...
variant to conditionally style things based on
...
variants are available by creating a new variant:

@custom-variant

aria-asc (&[aria-sort="ascending"])

;

@custom-variant

aria-desc (&[aria-sort="descending"])
...
variant that doesn’t make sense to include in your project, or for more complex ARIA attributes that take specific values, use square brackets to generate a property on the fly using any arbitrary value:
...
ARIA state variants can also target parent and sibling elements using the
...
variant to conditionally apply styles based on
...
Alternatively, you can configure shortcuts for common data attributes you're using in your project by creating a new variant in the
...
variants to conditionally add styles in right-to-left and left-to-right modes respectively when building multi-directional layouts:
...
Remember, these variants are only useful if you are building a site that needs to support

both

left-to-right and right-to-left layouts. If you're building a site that only needs to support a single direction, you don't need these variants — just apply the styles that make sense for your content.
...
variant to conditionally add styles when a
...
"text-sm leading-6 font-semibold text-gray-900 select-none"
...
variant lets you style elements marked with the
...
variant in situations where you need to style direct children that you don’t have control over:
...
variant can be used to style children of an element. The main difference is that
...
descendants, not just the direct children. This is especially useful when you combine it with another variant for narrowing the thing you're selecting:
...
Arbitrary variants are just format strings that represent the selector, wrapped in square brackets. For example, this arbitrary variant changes the cursor to

grabbing

when the element has the
...
If you need spaces in your selector, you can use an underscore. For example, this arbitrary variant selects all
...
You can also use at-rules like
...
If you find yourself using the same arbitrary variant multiple times in your project, it might be worth creating a custom variant using the
...
adding custom variants documentation
...
This is a comprehensive list of examples for all the pseudo-class variants included in Tailwind to complement the

pseudo-classes documentation
...
Style an element when it has focus using the
...
Style an element when it or one of its descendants has focus using the
...
Style an element when it has been focused using the keyboard using the
...
Style an element when it is being pressed using the
...
Style a link when it has already been visited using the
...
Style an element if its ID matches the current URL fragment using the
...
Style an element if it's the first child using the
...
Style an element if it's the last child using the
...
Style an element if it's the only child using the
...
Style an element if it's an oddly numbered child using the
...
Style an element if it's an evenly numbered child using the
...
Style an element if it's the first child of its type using the
...
Style an element if it's the last child of its type using the
...
Style an element if it's the only child of its type using the
...
Style an element at a specific position using the
...
Style an element at a specific position from the end using the
...
Style an element at a specific position, of the same type using the
...
Style an element at a specific position from the end, of the same type using the
...
nth-last-of-type-3:mx-6

nth-last-of-type-[3n+1]:mx-7
...
Style an element if it has no content using the
...
Style an input when it's disabled using the
...
Style an input when it's enabled using the
...
Style a checkbox or radio button when it's checked using the
...
Style a checkbox or radio button in an indeterminate state using the
...
Style an option, checkbox or radio button that was the default value when the page initially loaded using the
...
Style an input when it's optional using the
...
Style an input when it's required using the
...
Style an input when it's valid using the
...
Style an input when it's invalid using the
...
Style an input when it's valid and the user has interacted with it, using the
...
Style an input when it's invalid and the user has interacted with it, using the
...
Style an input when its value is within a specified range limit using the
...
Style an input when its value is outside of a specified range limit using the
...
Style an input when the placeholder is shown using the
...
Style an input when it has been autofilled by the browser using the
...
Style an input when it is read-only using the
