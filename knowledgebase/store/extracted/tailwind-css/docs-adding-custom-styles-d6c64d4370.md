...
collected_at: 2026-04-16T03:22:05.527396+00:00
kind: extracted-doc
---
...
- 2026-04-16T03:22:05.527396+00:00
...
- Adding custom styles - Core concepts - Tailwind CSS v 4.2 ⌘K Ctrl K Docs Blog Showcase Sponsor Plus Core concepts Adding custom styles Core concepts Adding custom styles Best practices for adding your own custom styles in Tailwind projects.
...
like inline styles, but again with the benefit that you can use modifiers:
...
are like arbitrary values but for doing on-the-fly selector modification, like you can with built-in pseudo-class variants like

hover:{utility}

or responsive variants like
...
If you're using something like JSX where the backslash is stripped from the rendered HTML, use
...
directive to add those styles to Tailwind's
...
Using Tailwind you probably don't need these types of classes as often as you think. Read our guide on
...
It will also work with variants like
...
Custom utilities are automatically inserted into the
...
In addition to registering simple utilities with the
...
This will match utilities like
...
To resolve the value as a bare value, use the
...
Available bare value data types are:
...
To support literal values, use the

--value('literal')

syntax (notice the quotes):
...
To support arbitrary values, use the
...
Available arbitrary value data types are:
...
function which works exactly like the
...
data type. If this is used with
...
In addition to using the variants that ship with Tailwind, you can also add your own custom variants using the
