...
collected_at: 2026-04-16T03:20:37.514118+00:00
kind: extracted-doc
---
...
- 2026-04-16T03:20:37.514118+00:00
...
to surface semantically relevant results. Importantly, this includes results with few or no shared keywords, which classical search techniques might miss.
...
, which we cover in detail later in the guide. This section will focus on the mechanics of semantic search.

Performing semantic search

You can query a vector store using the
...
in natural language. This will return a list of results, each with the relevant chunks, similarity scores, and file of origin.
...
A response will contain 10 results maximum by default, but you can set up to 50 using the
...
Certain query styles yield better results, so we’ve provided a setting to automatically rewrite your queries for optimal performance. Enable this feature by setting
...
The rewritten query will be available in the result’s
...
Attribute filtering helps narrow down results by applying criteria, such as restricting searches to a specific date range. You can define and combine criteria in

attribute_filter

to target files based on their attributes before performing semantic search.
...
to combine multiple filters using
...
If you find that your file search results are not sufficiently relevant, you can adjust the

ranking_options

to improve the quality of responses. This includes specifying a
...
between 0.0 and 1.0. A higher

score_threshold

will limit the results to more relevant chunks, though it may exclude some potentially useful ones. When

ranking_options.hybrid_search

is provided you can also tune
...
hybrid_search.text_weight

(

rrf_text_weight

) to control how reciprocal rank fusion balances semantic embedding matches vs. sparse keyword matches. Increase the former to emphasize semantic similarity, increase the latter to emphasize textual overlap, and ensure at least one of the weights is greater than zero.

Vector stores

Vector stores are the containers that power semantic search for the Retrieval API and the
...
Represents content uploaded through the

Files API

. Often used with vector stores, but also for fine-tuning and other use cases.
...
Wrapper type specifically representing a

file

that has been chunked and embedded, and has been associated with a
...
, are asynchronous and may take time to complete — use our helper functions, like

create_and_poll

to block until it is. Otherwise, you may check the status. Removing files from a vector store is eventually consistent, and search results may still include content from a removed file for a short period.

Adding files is rate limited per vector store ID. Requests to
...
share a per-vector-store limit of 300 requests per minute.
...
When creating a batch you can either provide
...
array to pass objects that include a
...
for each file. The two options are mutually exclusive so that you can cleanly control whether every file shares the same settings or you need per-file overrides.
...
, a dictionary of values that can be referenced when performing
...
You can set an expiration policy on
...
. Once a vector store expires, all associated
...
must be between 100 and 4096 inclusive.

chunk_overlap_tokens

must be non-negative and should not exceed
